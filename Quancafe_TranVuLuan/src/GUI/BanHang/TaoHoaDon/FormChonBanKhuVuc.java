...
public class FormChonBanKhuVuc extends JPanel {
...
//    private JPanel pnBan;
	...
	public FormChonBanKhuVuc() {
		initComponents();
		loadKhuVuc();
	}

	public FormChonBanKhuVuc(NhanVien nhanvienthungan) {
			...
			if (ban.getStatus() == 1) {
				...
				btnBan[index].addMouseListener(new MouseAdapter() {
					...
					public void mouseClicked(MouseEvent e) {
						int rs = JOptionPane.showConfirmDialog(null, "Bàn đang bận, bạn muốn gộp bàn");
						if (rs == 0)
							goToOrder(ban.getMaBan(), ban.getTenBan());
					}
				...
				btnBan[index].addMouseListener(new MouseAdapter() {
					...
					public void mouseClicked(MouseEvent e) {
						...
						if (rs == 0) {
							...
							goToOrder(ban.getMaBan(), ban.getTenBan(), dsMon);
						}
					}

				});
			}else if (ban.getStatus() == 3) {
				btnBan[index].setBackground(new Color(72, 0, 0));
				btnBan[index].addMouseListener(new MouseAdapter() {
					@Override
					public void mouseClicked(MouseEvent e) {
						int rs = JOptionPane.showConfirmDialog(null, "Bàn hiện khoá");
					}
	...
	public void goToOrder(int ma_ban, String ten_ban, Vector<Vector> dsMon) {
		FormChonMon formChonMon = new FormChonMon(ma_ban, ten_ban, tenKhuVuc, nhanvienthungan, dsMon);
		this.remove(pnBan);
		this.remove(pnKhuVuc);
		this.add(formChonMon);
		this.revalidate();
